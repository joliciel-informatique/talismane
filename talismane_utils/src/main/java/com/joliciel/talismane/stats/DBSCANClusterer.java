...
/**
 * Performs clustering on a dataset using the DBSCAN algorithm and Euclidean
...
public class DBSCANClusterer<T> {
   ...
   * @param dataSet
   *          the data vectors describing the objects to be clustered -
   *          clustering will be performed using the Euclidean distance data
   *          vectors
   */
   ...
   * 
   * @param epsilon
   *          the maximum distance for including two points in the same cluster
   * @param minPoints
   *          the minimum points required to form a cluster
   * @param includeNoise
   *          whether or not noise should be included when returning the
   *          clusters
   */
  public Set<Set<T>> cluster(double epsilon, int minPoints, boolean includeNoise) {
    ...
    /*
     * DBSCAN(D, eps, MinPts) C = 0 for each unvisited point P in dataset D mark
     * P as visited N = getNeighbors (P, eps) if sizeof(N) < MinPts mark P as
     * NOISE else C = next cluster expandCluster(P, N, C, eps, MinPts)
     */
  ...
  private void expandCluster(int index, Set<Integer> neighbours, Set<T> cluster, double epsilon, int minPoints) {
    /*
     * expandCluster(P, N, C, eps, MinPts) add P to cluster C for each point P'
     * in N if P' is not visited mark P' as visited N' = getNeighbors(P', eps)
     * if sizeof(N') >= MinPts N = N joined with N' if P' is not yet member of
     * any cluster add P' to cluster C
     */
