...
/**
 * In the case where the FScoreCalculator is used to generate many confusion matrices for different
 * configurations, where each configuration has a different file name but is located in the same directory,
 * this class generates a single CSV file per directory comparing the accuracy scores of all of the different matrices.
 * @author Assaf Urieli
 *
 */
public class AccuracyComparison {
	...
	public static void main(String[] args) throws Exception {
		...
		Writer writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(new File(directory, directory.getName() + fileExtensionBase + "_totalAccuracy" + fileExtensionSuffix)), "UTF-8"));
		writer.write(CSV.format("File") 
				+ CSV.format("Accuracy") 
				+ CSV.format("good") 
				+ CSV.format("bad") 
				+ CSV.format("total"));
		...
		for (String fileSuffix : confusionMatrixes.keySet()) {
			...
			Writer matrixWriter = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(new File(directory, directory.getName() + fileExtensionBase + fileSuffix + fileExtensionSuffix)), "UTF-8"));
			...
			for (@SuppressWarnings("unused") String rowName : confusionMatrix.keySet()) {
	...
	private static void writeAccuracy(File directory) throws Exception {
		...
		Writer writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(new File(directory, directory.getName() + fileExtensionBase + "_accuracy" + fileExtensionSuffix)), "UTF-8"));
		writer.write(CSV.format("File") 
				+ CSV.format("Accuracy") 
				+ CSV.format("good") 
				+ CSV.format("bad") 
