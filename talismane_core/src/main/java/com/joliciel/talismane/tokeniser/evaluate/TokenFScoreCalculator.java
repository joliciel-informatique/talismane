 ...
 *
 */
public class TokenFScoreCalculator implements TokenEvaluationObserver {
  private static final Logger LOG = LoggerFactory.getLogger(TokenFScoreCalculator.class);
  private static final CSVFormatter CSV = new CSVFormatter(4);
  private static final int NUM_CHARS = 20;
  private final FScoreCalculator<TokeniserOutcome> fScoreCalculator = new FScoreCalculator<TokeniserOutcome>();

  private final Map<String, FScoreCalculator<TokeniserOutcome>> taggerFScoreCalculators = new TreeMap<String, FScoreCalculator<TokeniserOutcome>>();
  private final Map<String, List<TokeniserErrorRecord>> errorMap = new TreeMap<String, List<TokeniserErrorRecord>>();
  private final Writer errorWriter;
  private final Writer csvErrorWriter;
