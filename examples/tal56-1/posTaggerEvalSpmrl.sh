...
java -Xmx16G -jar talismane-fr-2.4.2b.jar languagePack=frenchLanguagePack-2.4.1b.zip command=evaluate startModule=posTagger endModule=posTagger posTaggerModel=models/${PosTaggerModel}.zip inFile=${CORPUS} corpusReader=${CorpusReader} outDir=eval/posTagger/${CORPUSCODE} beamWidth=${BEAM} suffix=_Pos${TagModelType}${RULES}_beam${BEAM} maxParseAnalysisTime=0 predictTransitions=false posTagSet=spmrl2013patches/spmrlTagset.txt logConfigFile=conf/log4j.properties performanceConfigFile=conf/performance.properties
