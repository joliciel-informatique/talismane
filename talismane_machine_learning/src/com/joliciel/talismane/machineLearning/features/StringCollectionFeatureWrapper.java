...
class StringCollectionFeatureWrapper<T> extends AbstractMonitorableFeature<T,List<WeightedOutcome<String>>> implements
	...
	private static final PerformanceMonitor MONITOR = PerformanceMonitor.getMonitor(StringCollectionFeatureWrapper.class);
	...
	public FeatureResult<List<WeightedOutcome<String>>> checkInternal(T context, RuntimeEnvironment env) {
		...
		List<FeatureResult<List<WeightedOutcome<String>>>> collectionResultList = null;
		MONITOR.startTask("collection results");
		try {
			// get the collection results for each enclosed collection
			collectionResultList = new ArrayList<FeatureResult<List<WeightedOutcome<String>>>>();
		...
		if (collectionResultList.size()>0) {
			List<List<CollectionFeatureResult>> crossProduct = null;
			MONITOR.startTask("cross product");
			try {
				// we do a cross product of all of the results from all of the enclosed collections
				crossProduct = new ArrayList<List<CollectionFeatureResult>>();
			...
			MONITOR.startTask("test wrapped feature");
			try {
				for (List<CollectionFeatureResult> oneCollectionResultSet : crossProduct) {
					...
					MONITOR.startTask("build final list");
					try {
