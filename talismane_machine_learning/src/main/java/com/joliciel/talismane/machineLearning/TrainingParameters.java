...
/**
 * A class storing everything required to run a machine learning training
 * session. Possible keys in props:<br/>
 * <ul>
 ...
 * <li><i>prefix</i>.balanceEventCounts: whether or not event counts should be
 * balanced (linear SVM only)</li>
 * <li><i>prefix</i>.features: points at a relative file path where feature
 * desciptors can be found</li>
 * </ul>
 * The prefix is optional - if no prefix is provided, no . is added before the
 * property name.
 * 
 * @author Assaf Urieli
 *
...
public class TrainingParameters {
	 ...
	 * 
	 * @param prefix
	 *            if not null and not empty, will be prefixed to each property
	 ...
	 *            if not null, these will give default parameters which can be
	 *            overridden by the current set of properties.
	 * @param prefix
	 *            if not null and not empty, will be prefixed to each property
	 *            name with an additional "."
	 */
	...
	/**
	 * Load a given set of properties with paths relative to a given base
	 * directory, and return a set of all property names processed.
	 * 
	 * @param baseDir
	 *            The directory relative to which paths in the property list
	 *            should be interpreted. If null, current working directory is
	 *            used.
	 * @param props
	 *            The list of properties to interpret.
	...
	public Set<String> load(File baseDir, Map<String, String> props) {
		try {
			...
			if (algorithm != null) {
				if (algorithm == MachineLearningAlgorithm.MaxEnt) {
					...
					if (props.containsKey(propPrefix + "linearSVMSolver")) {
						parameters.put(LinearSVMModelTrainer.LinearSVMModelParameter.SolverType.name(),
								LinearSVMSolverType.valueOf(props.get(propPrefix + "linearSVMSolver")));
						propNames.add(propPrefix + "linearSVMSolver");
					}
					if (props.containsKey(propPrefix + "linearSVMCost")) {
						parameters.put(LinearSVMModelTrainer.LinearSVMModelParameter.ConstraintViolationCost.name(),
								Double.parseDouble(props.get(propPrefix + "linearSVMCost")));
						propNames.add(propPrefix + "linearSVMCost");
					}
					if (props.containsKey(propPrefix + "linearSVMEpsilon")) {
						parameters.put(LinearSVMModelTrainer.LinearSVMModelParameter.Epsilon.name(),
								Double.parseDouble(props.get(propPrefix + "linearSVMEpsilon")));
						propNames.add(propPrefix + "linearSVMEpsilon");
					}
					if (props.containsKey(propPrefix + "balanceEventCounts")) {
						parameters.put(LinearSVMModelTrainer.LinearSVMModelParameter.BalanceEventCounts.name(),
								"true".equalsIgnoreCase(props.get(propPrefix + "balanceEventCounts")));
			...
			if (props.containsKey(propPrefix + featureProperty)) {
				...
				try (Scanner featureScanner = new Scanner(new BufferedReader(new InputStreamReader(new FileInputStream(featureFile), "UTF-8")))) {
					featureDescriptors = new ArrayList<String>();
