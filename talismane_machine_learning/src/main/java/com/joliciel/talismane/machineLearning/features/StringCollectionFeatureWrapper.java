...
class StringCollectionFeatureWrapper<T> extends AbstractFeature<T, List<WeightedOutcome<String>>>implements StringCollectionFeature<T> {
  ...
  public FeatureResult<List<WeightedOutcome<String>>> check(T context, RuntimeEnvironment env) throws TalismaneException {
    ...
    if (collectionResultList.size() > 0) {
      ...
      // Test the wrapped feature for each set of collection results in
      // the cross-product
        ...
        if (featureResult != null) {
          if (wrappedFeature.getFeatureType().equals(StringFeature.class)) {
            ...
            throw new JolicielException(
                "Cannot include collections in a top-level feature of type: " + wrappedFeature.getFeatureType().getSimpleName());
          }
        }
      }
      if (finalList.size() > 0)
