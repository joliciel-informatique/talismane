...
class FunctionDescriptorParserImpl implements FunctionDescriptorParser {
	...
	public FunctionDescriptor parseDescriptor(String text) {
		...
		FunctionDescriptor rootDescriptor = this.featureServiceInternal.getFunctionDescriptor(ROOT_NAME);
		...
		for (int i=0;i<text.length();i++) {
			...
			} else if (context.c=='+'||context.c=='-'||context.c=='<'||context.c=='>'||context.c=='='
				||context.c=='!'||context.c=='*'||context.c=='/'||context.c=='%'||context.c=='&'||context.c=='|') {
	...
	void doOpenParentheses(FunctionDescriptorParseContext context) {
		...
		FunctionDescriptor openDescriptor = this.featureServiceInternal.getFunctionDescriptor(context.currentString);
	...
	/**
	 * If operator is unary, add it to current string and do nothing else.
	...
	void doOperator(FunctionDescriptorParseContext context) {
		...
		if (!unaryOperator) {
			...
			if (context.lastCharacterClass.equals(CharacterClass.OPERATOR)) {
				...
				FunctionDescriptor operator = this.featureServiceInternal.getFunctionDescriptor("" + context.c);
	...
	/**
	 * If current string length > 0, add it as argument to top descriptor.
	 * If top descriptor is binary, pop it off the stack.
	 */
	...
	void addArgument(FunctionDescriptorParseContext context, boolean asString) {
		...
		if (asString || context.currentString.length()>0) {
			...
			if (asString) {
				argument = this.featureServiceInternal.getFunctionDescriptorForObject(context.currentString);
			} else {
				argument = this.featureServiceInternal.getFunctionDescriptorForArgument(context.currentString);
			}
