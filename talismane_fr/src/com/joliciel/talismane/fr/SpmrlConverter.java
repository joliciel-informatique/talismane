...
public class SpmrlConverter {
	...
	public static void main(String[] args) throws Exception {
		...
		File directory = spmrlFile.getParentFile();
		String fileName = spmrlFile.getName().substring(0, spmrlFile.getName().length()-".conll".length()) + "." + suffix + ".conll";
		Writer writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(new File(directory, fileName)), "UTF-8"));
		Scanner scanner = new Scanner(new BufferedReader(new InputStreamReader(new FileInputStream(spmrlFile), "UTF-8")));
			...
			if (line.trim().length()==0) {
				...
				for (TokenCluster tokenCluster : tokens) {
					...
					String newLine = tokenCluster.newIndex + "\t" + tokenCluster.word + "\t" + tokenCluster.lemma + "\t"
							+ conllLine.posTag + "\t" + conllLine.posTag2 + "\t" + conllLine.morph + "\t"
							+ indexMap.get(conllLine.governor) + "\t" + conllLine.label + "\t"
							+ indexMap.get(conllLine.projGov) + "\t" + conllLine.projLabel;
					ConllLine newConllLine = new ConllLine(newLine, newLineNumber);
				...
				ConllLine conllLine = new ConllLine(line, lineNumber);
				lines.add(conllLine);
			}
		}
	...
	private static final class ConllLine {
		...
		public String toString() {
			String string = this.index + "\t" + this.word + "\t" + this.lemma + "\t"
				+ this.posTag + "\t" + this.posTag2 + "\t" + this.morph + "\t"
				+ this.governor + "\t" + this.label + "\t"
				+ this.projGov + "\t" + this.projLabel;
			return string;
		}
	}
}
