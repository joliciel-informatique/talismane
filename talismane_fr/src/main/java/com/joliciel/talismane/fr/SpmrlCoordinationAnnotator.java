...
public class SpmrlCoordinationAnnotator {
  ...
  public void calc(Map<String, String> argMap) throws Exception {
    ...
    case None:
      suffix = "1H";
      break;
    case PrevConjunctGov:
      suffix = "PH";
      break;
    case PrevConjunctGovNoCommas:
      suffix = "PH2";
      break;
    }
    ...
    try (Writer writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(outFile), "UTF-8"));
      ...
      while (refFileScanner.hasNextLine()) {
        ...
        if (nextRefLine.length() == 0 || !refFileScanner.hasNextLine()) {
          ...
          // first we fix the conjuncts so that we're sure each
          // depends on the previous conjunct
          for (int j = 1; j < refLines.size(); j++) {
              ...
              if (coordinatorCount > 1) {
                ...
                for (ConllLine coordinator : coordinators) {
                  if (coordinator.label.equals("coord")) {
                    ...
                    // non coordinators, retain
                    // non-projective dependency on first
                    // conjunct just in case it was right
                    coordinator.governor = lastConjunct.index;
                  }
                }
          ...
          if (option == Option.ConjunctionGov) {
            for (int j = 1; j < refLines.size(); j++) {
              ...
              ConllLine refLine = refLines.get(j);
              // first decide if the refLine is the last
              // coordinator in a coordination (either the
              // conjunction, or the last comma)
              boolean isLastCoordinator = false;
              if (refLine.posTag.equals("CC") && (refLine.label.equals("coord") || refLine.label.equals("root"))) {
                // we need to exclude the second conjunction in
                // et/ou, which has no conjuncts
